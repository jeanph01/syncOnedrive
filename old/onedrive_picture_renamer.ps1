...
function initialisation {
    # charge le fichier extrait par onedrivesync.ps1
    if (!(Test-Path $IndexFile)) { 
        Write-Error "Cache introuvable. Executer onedrivesync.ps1"; 
        exit 
    }
    ...
    # initialise ou charge le fichier du cache gps/nom de localisation
    $script:GpsCache = @{}
    if (Test-Path $GpsCacheFile) { 
        $script:GpsCache = Get-Content $GpsCacheFile | ConvertFrom-Json -AsHashtable 
    }
} # initialisation
